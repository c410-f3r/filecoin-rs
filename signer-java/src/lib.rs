...
pub extern "system" fn Java_Signer_generate_1mnemonic(env: JNIEnv, _class: JClass) -> jstring {
    ...
    let output = env.new_string(mnemonic.0)
...
#[no_mangle]
pub extern "system" fn Java_Signer_key_1derive(env: JNIEnv, _class: JClass, mnemonic: JString, path: JString) -> jobject {

    let mnemonic_rust: String =
     env.get_string(mnemonic).expect("Couldn't get the mnemonic java string!").into();

    let path_rust: String =
     env.get_string(path).expect("Couldn't get the path java string!").into();

     let ext_key = filecoin_signer::key_derive(filecoin_signer::Mnemonic(mnemonic_rust), path_rust).unwrap();
